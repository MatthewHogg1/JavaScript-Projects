...
const Calculator = {
    ...
    Display_Value: '0',
    //This will hold the first ground operand for any expressions, we set it to null for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    //This checks if the Wait_Second_operand is true and set Display_Value to the key that was clicked on
...
function Handle_Operator(Next_Operator) {
    ...
    //When an operator key is pressed we convert the current number displayed to a number and then store the result in Calculator.First_Operand and if it doesnt exist
    const Value_of_Input = parseFloat(Display_Value);
    //Checks if an operator already exists and if Wait_Second_operand is true, then updates the operator an exits the function
...
keys.addEventListener('click', (event) => {
    //The target variable is an object that represeents the element that was clicked
    const { target } = event;
